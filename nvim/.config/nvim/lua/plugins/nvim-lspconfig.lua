return {
  ...
  config = function()
    ...
    local function is_leetcode_path(path)
      if not path or path == '' then
        return false
      end

      local normalized = vim.fs.normalize(path)
      return normalized:find(leetcode_home, 1, true) == 1
    end
    ...
    local servers = {
      ...
      ruby_lsp = {
        ...
        root_dir = function(bufnr, on_dir)
          ...
          if is_leetcode_path(bufname) then
            return
          end

          on_dir(vim.fs.root(bufnr, { 'Gemfile', '.git' }) or vim.fs.dirname(bufname))
        end,
        init_options = {
