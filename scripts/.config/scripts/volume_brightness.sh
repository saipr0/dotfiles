...
volume_step=1
brightness_step=2  # Percentage step for brightness
max_volume=100
...
function get_mute {
    pactl set-sink-mute @DEFAULT_SINK@ | grep -Po '(?<=Mute: )(yes|no)'
}

# Get brightness percentage using brightnessctl -m
function get_brightness {
    brightnessctl -m | awk -F',' '{print $4}' | tr -d '%'
}

# Adjust brightness by a percentage using brightnessctl -m
function set_brightness {
    # $1 is the percentage change (e.g., +5 or -5)
    current_percent=$(get_brightness)
    new_percent=$(( current_percent + $1 ))
...
case $1 in
    ...
    set_brightness +${brightness_step}
